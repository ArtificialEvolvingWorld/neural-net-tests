...
TwoPendulumEntity::TwoPendulumEntity(TwoPendulum initial)
  : rk(initial, 0.01),
    base(nullptr), base_position(0,0,0),
    cart(nullptr), rod1(nullptr), rod2(nullptr) { }

TwoPendulumEntity::~TwoPendulumEntity() {
  if(cart) {
    cart->remove();
  }
  if(rod1) {
...
void TwoPendulumEntity::set_position(irr::core::vector3df pos) {
  base_position = pos;
  if(base) {
...
void TwoPendulumEntity::initialize(irr::IrrlichtDevice* device) {
...
void TwoPendulumEntity::update(double deltaT, const EventReceiver& /*events*/) {
  rk.Step();
